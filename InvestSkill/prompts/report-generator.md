...
> [!IMPORTANT]
> **Report Signal Color Theme Rules (强制主题配色规范):**
> - **Bullish (看多 / BUY / STRONG BUY)**: Primary theme, hero gradient, accent borders, badges, table highlights MUST use Green / Emerald (`#059669`, `#10B981`, `#047857`, emerald gradients).
> - **Neutral (中立 / HOLD)**: Primary theme, hero gradient, accent borders, badges, table highlights MUST use Amber / Orange (`#D97706`, `#F59E0B`, `#B45309`, amber/orange gradients).
> - **Bearish (看空 / SELL / STRONG SELL)**: Primary theme, hero gradient, accent borders, badges, table highlights MUST use Red / Crimson (`#DC2626`, `#EF4444`, `#991B1B`, red gradients).
...
- **Dynamic Output Index Requirement**: Whenever generating or updating an HTML report in `output/`, the agent **MUST** update `output/index.html` dynamically (e.g. by running `node scripts/generate-output-index.js` or `npm run update:index`). `output/index.html` serves as an interactive directory allowing users to view and navigate to all generated HTML reports.
