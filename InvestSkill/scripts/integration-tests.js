...
function testPlatformArtifacts() {
  test('Plugin manifest exists', () => {
    ...
    if (!plugin.name) throw new Error('No name in plugin.json');
  });
...
function testNoDuplicateSkills() {
  ...
  test('No duplicate skills in registry', () => {
    const uniqueSkills = new Set(skillDirs);
    if (skillDirs.length !== uniqueSkills.size) {
      throw new Error(`Duplicate skills detected (${skillDirs.length} total, ${uniqueSkills.size} unique)`);
    }
  });
